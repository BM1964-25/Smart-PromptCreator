...
export function PromptList({ prompts, categories, selectedId, onSelect }: PromptListProps) {
  return (
    <div className="min-w-0 border-r border-line bg-[#f9f8f3] dark:border-[#333] dark:bg-[#1c1c1b]">
      <div className="border-b border-line px-4 py-3 dark:border-[#333]">
        <h2 className="text-sm font-semibold">Prompts</h2>
        <p className="text-xs text-neutral-500">{prompts.length} Eintraege</p>
      </div>
      <div className="h-[calc(100vh-61px)] overflow-auto p-2">
        {prompts.map((prompt) => {
